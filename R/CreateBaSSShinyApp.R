#' @name  CreateBaSSShinyApp
#' @title  CreateBaSSShinyApp
...
CreateBaSSShinyApp <- function(strProjectDirectory, strShinyAppName, strShinyAppDisplayName, strCalculationLibraryName, bCreateProjectSubdirectory = TRUE, bCreateWithExampleTabs = TRUE )
{
    ...
    if( bCreateWithExampleTabs )
        strAppDirectory         <- paste( strTemplateDirectory, "/RShinyApp", sep="" )
    else
        strAppDirectory         <- paste( strTemplateDirectory, "/RShinyAppNoTabs", sep="" )
